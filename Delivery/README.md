...
- **Juice:** "+points" labels (pop-in, wobble, rainbow, rise, fade), coins arcing into the counter with a punch on arrival, milestone cheers (pop, flash, wobble, 2 s), a mascot that hops idly and jumps on each shot, a screen shake on clears (scaled by bubbles cleared, stronger for row wipes), an impact ripple (damped squash & stretch) on the landed bubble and its neighbors, an accelerating shot (ease-in from 35% to top speed — speed only, so the aim guide stays exact), the "next" indicator jumping in an arc onto the "current" slot on each shot, a lever backfire (kick back opposite the aim + squash along the barrel, damped spring), a shot trail (copies of the bubble, siblings drawn right behind the shot, each replaying where it was a few hundredths of a second earlier — shrinking and fading, bending with wall bounces and catching up on landing), and a "line clear" sweep: when a special bubble pops, two copies spawn on it and rush to the left and right edges of its row (accelerating, stretching, fading). All deterministic (no random motion) and hand-animated.
